...
cat("sigma_portefeuille", sigma_portefeuille)
ratio_sharpe = mean(rendement_moy_port*255-0.007)/(sigma_portefeuille*sqrt(255))
cat("sigma_portefeuille", ratio_sharpe)
```

## Test de normalité des rendement avec qqplot
...
## VaR à 99% historique et VaR théorique
...
cat("VaR à 99% historique: ", varhistoriq)

vargauss = VaR(rendement_histo_port,p=0.99,method = "gaussian")

cat("VaR à 99% par la loi normale: ", vargauss)
```

## CTE à partir de la VaR historique
...
cat("Volatilité maximale observées , de toute les simulations: ", vola_proj_max)
